...
int check4k(int die[],unsigned short numdice,unsigned short &totscore){
    ...
    unsigned short four=4;
            cout<<"Enter one die to score for 4 of a kind"<<endl;
            cin>>select;
            
            for(int i=0;i<numdice;i++){
                if(select==i+1){              //Select value to check matches
                    value=die[i];
                }
            }
            for(int i=0;i<numdice;i++){
                if(die[i]==value){           //If value chosen is equal to die, count match
                    match+=1;
                }
            }
            //Score 4 of a kind
...
int scrAce(int die[],unsigned short numdice){
    ...
    for(int i=0;i<numdice;i++){
        if(die[i]==1)           //Score dice only with value of one (ace)
            acescre+=1;
    }
...
void roll1(int die[],unsigned short numdice){
    ...
    //Output prompt for dice to hold
            ...
            cin>>select1>>select2>>select3>>select4;
